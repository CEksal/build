...
void main() {
  ...
  final webTarget = DefaultBuildTarget(
    (b) =>
        b
          ..target = 'web'
...
main() {
  ...
  group('Build Daemon', () {
    ...
    test(
      'shuts down on build script change',
    ...
    test(
      ...
      () async {
        ...
        // We need to add a listener otherwise we won't get the event.
        unawaited(
          client.shutdownNotifications.first.then(
    ...
    test('errors if build modes conflict', () async {
      await startDaemon();
      expect(
        startClient(buildMode: BuildMode.Manual),
    ...
    test('can build in manual mode', () async {
      ...
      expect(
        client.buildResults,
        emitsThrough(
          (BuildResults b) => b.results.first.status == BuildStatus.succeeded,
        ),
      );
    });
  ...
  print('goodbye world');
    ...
    test('can build to outputs', () async {
      ...
      var client =
          await startClient()
            ...
            ..startBuild();
      clients.add(client);
      await client.buildResults.firstWhere(
        (b) => b.results.first.status == BuildStatus.succeeded,
      );
    ...
    test('notifies upon build start', () async {
      ...
      expect(
        client.buildResults,
        emitsThrough(
          (BuildResults b) => b.results.first.status == BuildStatus.started,
        ),
      );
